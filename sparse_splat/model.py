...
@dataclass
class SparseSplatModelConfig(SplatfactoModelConfig):
    """Splatfacto Model Config, nerfstudio's implementation of Gaussian Splatting"""

    _target: Type = field(default_factory=lambda: SparseSplatModel)
...
class SparseSplatModel(SplatfactoModel):
    ...
    def populate_modules(self):
        ...
        self.sparsity_weight_max = 2e-2
        self.max_step = 20000
        self.sparsity_weight_diff = self.sparsity_weight_max - self.sparsity_weight
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        sparsity_loss = torch.nn.functional.l1_loss(torch.sigmoid(self.opacities),
                                                    torch.zeros_like(self.opacities).to(self.device))
        
        sparsity_loss = torch.abs(torch.sigmoid(self.opacities)).mean()
        
        self.sparsity_weight = torch.minimum(torch.tensor(self.sparsity_weight + (1 / self.max_step) * self.sparsity_weight_diff),
                                             torch.tensor(self.sparsity_weight_max)).to(self.device)
        ...
        loss_dict["main_loss"] = loss_dict["main_loss"] + self.sparsity_weight * sparsity_loss
